...
const citiesControllers = {
    ...
    removeACity:(req, res) =>{
        if(typeof req.body._id === "string"){
            ...
            .then(city => {
                city ?  res.json({success: true, response: city}) : res.json({success: false, response: "didn't find desired city"})               
            })
            .catch(err => res.json({success: false, response: err}))
            // pedir al modelo que elimine la ciudad que requeri.
        }else{
            res.json({success:false, reason:"id is not a string"})
        }
    },
    modifyACity:(req, res)=>{
        if(typeof req.body._id === "string"){
            ...
            res.json({success: false, response: "id is not a string"})
        }
    }
}
